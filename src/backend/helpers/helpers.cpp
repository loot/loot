...
namespace loot {
    ...
    uint32_t GetCrc32(const fs::path& filename) {
        ...
        try {
            ...
            throw Error(Error::Code::path_read_fail, (boost::format(lc::translate("Unable to open \"%1%\" for CRC calculation.")) % filename.string()).str());
        }
    ...
    void OpenInDefaultApplication(const boost::filesystem::path& file) {
        ...
        if ((int)ret <= 32)
            throw Error(Error::Code::windows_error, lc::translate("Failed to open file in its default application."));
#else
