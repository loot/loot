...
std::optional<std::filesystem::path> ReadPathFromRegistry(
  ...
  try {
    ...
    const auto gamePath = std::filesystem::u8path(installedPath);
    ...
    if (registryKey == NEHRIM_STEAM_REGISTRY_KEY) {
    ...
    const auto logger = getLogger();
    if (logger) {
      logger->error("Error while trying to look up the registry key \"{}\": {}",
                    registryKey,
                    e.what());
    }
  }
...
std::vector<std::filesystem::path> FindGameInstallPathsInRegistry(
  ...
  for (const auto& registryKey : registryKeys) {
    const auto path = ReadPathFromRegistry(registryKey);
