...
TEST_P(FindGamePathsTest,
       shouldSetGameLocalPathIfNonGOGEnderalSEIsFoundAndNoLocalPathIsSet) {
  ...
  EXPECT_EQ(settings.GamePath(), gamePaths.value().installPath);
  EXPECT_EQ("Enderal Special Edition",
            gamePaths.value().localPath.stem().u8string());
}

TEST_P(FindGamePathsTest,
       shouldSetGameLocalPathIfGOGEnderalSEIsFoundAndNoLocalPathIsSet) {
  if (GetParam() != GameType::tes5se) {
    return;
  }

  touch(dataPath.parent_path() / "Enderal Launcher.exe");
  touch(dataPath.parent_path() / "Galaxy64.dll");
  ...
  EXPECT_EQ(settings.GamePath(), gamePaths.value().installPath);
  EXPECT_EQ("Enderal Special Edition GOG",
            gamePaths.value().localPath.stem().u8string());
}

TEST_P(FindGamePathsTest,
       shouldNotOverrideGameLocalPathIfEnderalSEIsFoundAndLocalPathIsSet) {
  if (GetParam() != GameType::tes5se) {
    return;
  }
...
TEST_P(FindGamePathsTest, shouldNotSetGameLocalPathIfNonEnderalGameIsFound) {
  const auto settings =
      GameSettings(GetParam()).SetGamePath(dataPath.parent_path());
  ...
  EXPECT_EQ(settings.GamePath(), gamePaths.value().installPath);
  EXPECT_TRUE(gamePaths.value().localPath.empty());
}
